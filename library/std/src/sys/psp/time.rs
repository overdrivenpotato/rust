...
impl SystemTime {
    ...
    pub fn try_from_psp_time(t: &libc::ScePspDateTime) -> Result<SystemTime, ()> {
        let mut secs_since_epoch: i64 = 0;
        if unsafe {libc::sceRtcGetTime_t(t as *const libc::ScePspDateTime, &mut secs_since_epoch as *mut i64)} < 0 {
            Err(())
        } else {
            Ok(Self(Duration::from_secs(secs_since_epoch as u64)))
        }
    }
